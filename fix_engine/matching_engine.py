...
class MatchingEngine:
    ...
    def match_order(self, new_order: dict, dry_run: bool = False) -> list:
        ...
        with self._lock:
            matches, sym, side, px, rem = [], new_order["symbol"], str(new_order["side"]), float(new_order["price"]), float(new_order["qty"])
            ...
            while i < len(opp_match) and rem > 0:
                maker = opp_match[i]
                if (side == "1" and px < float(maker["price"])) or (side == "2" and px > float(maker["price"])): break

                m_qty, f_px = min(rem, float(maker["qty"])), float(maker["price"])
                b_id, s_id, b_cid, s_cid = (new_order["id"], maker["id"], new_order.get("client_id", "K"), maker.get("client_id", "K")) if side == "1" else (maker["id"], new_order["id"], maker.get("client_id", "K"), new_order.get("client_id", "K"))
                
                matches.append({"buy_id": b_id, "sell_id": s_id, "buy_client_id": b_cid, "sell_client_id": s_cid, "qty": m_qty, "price": f_px, "symbol": sym})
                rem -= m_qty
                
                if not dry_run:
                    maker["qty"] -= m_qty
                    print(f"🔄 [MATCH] {sym}: {m_qty} @ {f_px} | buyer={b_id} seller={s_id}", flush=True)
                    if maker["qty"] <= 0:
                        opp_match.pop(i)
                        continue
                i += 1
