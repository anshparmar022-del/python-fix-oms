...
class OMSApp(fix.Application):
    ...
    def onLogon(self, sessionID):
        ...
        print(f"🏛️  [SESSION] Authenticated and Saved: {client_id} ({sessionID})", flush=True)
    ...
    def toAdmin(self, message, sessionID):
        ...
        mt = msgType.getValue()
        label = "Admin Message"
        if mt == fix.MsgType_Heartbeat:  label = "Heartbeat"
        elif mt == fix.MsgType_Logon:    label = "Logon Sent"
        elif mt == fix.MsgType_Logout:   label = "Logout Sent"
        elif mt == fix.MsgType_TestRequest: label = "Test Request"
    ...
    def fromAdmin(self, message, sessionID):
        ...
        label = "Admin Message"
        if mt == fix.MsgType_Heartbeat:  label = "Heartbeat"
        elif mt == fix.MsgType_Logon:    label = "Logon Received"
        elif mt == fix.MsgType_Logout:   label = "Logout Received"
        elif mt == fix.MsgType_TestRequest: label = "Test Request ACK"
        print(f"📥 [ADMIN] {label.ljust(15)} | {raw}", flush=True)
    ...
    def fromApp(self, message, sessionID):
        ...
        message.getHeader().getField(msgType)
        mt = msgType.getValue()
        label = {"D": "NewOrderSingle", "F": "OrderCancelReq", "G": "OrderReplaceReq", "q": "MassCancelReq", "AN": "PositionRequest"}.get(mt, "App Message")
        print(f"📥 [APP]   {label.ljust(15)} | {raw}", flush=True)
        try:
            if   mt == fix.MsgType_NewOrderSingle:           self._on_new_order(message, sessionID)
            elif mt == fix.MsgType_OrderCancelRequest:       self._on_cancel_request(message, sessionID)
            elif mt == fix.MsgType_OrderCancelReplaceRequest: self._on_replace_request(message, sessionID)
            elif mt == fix.MsgType_OrderMassCancelRequest:   self._on_mass_cancel_request(message, sessionID)
            elif mt == "AN":                                 self._on_position_request(message, sessionID)
        except Exception as e:
            logger.exception("fromApp failed: %s", e)
    ...
    def _on_new_order(self, message, sessionID):
        ...
        try:
            ...
            order_id, qty, px, sym, side = data["clord_id"], float(data["qty"]), float(data["price"]), data["symbol"], data["side"]
            client_id = self._get_client_id(message, sessionID)
            tif_f = fix.TimeInForce()
            tif = tif_f.getValue() if message.isSetField(tif_f) and message.getField(tif_f) else "0"
            
            print(f"📋 [ORDER] New | id={order_id} sym={sym} side={side} qty={qty} px={px}", flush=True)
            if order_id in self.order_tracker or manager.get_order(order_id):
                self._send_report(order_id, 0, 0, sym, side, "8", 0, text="Duplicate ClOrdID", sessionID=sessionID)
                return
            ...
            if tif == "3":
                if tr and tr["status"] not in ("2", "4"):
                    ...
                    self._update_bbo(sym)
        except Exception as e: logger.exception("_on_new_order failed: %s", e)
    ...
    def _on_cancel_request(self, message, sessionID):
        ...
        try:
            ...
            if engine.cancel_order(oid):
                ...
                self._update_bbo(sym)
            else: self._send_cancel_reject(nid, oid, "Unknown Order", sessionID)
        except Exception as e: logger.exception("_on_cancel_request failed: %s", e)
    ...
    def _on_mass_cancel_request(self, message, sessionID):
        ...
        try:
            ...
            else: [self._update_bbo(s) for s in manager.get_all_symbols()]
            rep = fix.Message()
            rep.getHeader().setField(fix.MsgType("r"))
            rep.setField(fix.StringField(530, "7"))
        ...
        except Exception as e: logger.exception("_on_mass_cancel_request failed: %s", e)
    ...
    def _on_replace_request(self, message, sessionID):
        ...
        try:
            ...
            if engine.cancel_order(oid):
                ...
                for m in engine.match_order(od): self._process_fill(m, sym, sessionID)
            else: self._send_cancel_reject(nid, oid, "Unknown Order", sessionID)
    ...
    def _process_fill(self, m: dict, sym, sessionID):
        ...
        for mid, side, cid in [(m["buy_id"], "1", m["buy_client_id"]), (m["sell_id"], "2", m["sell_client_id"])]:
            if mid not in self.order_tracker:
                s = manager.get_order(mid) or {}
                self.order_tracker[mid] = {"cum_qty": float(s.get("filled_qty", 0)), "total_qty": float(s.get("qty", fq)), "symbol": sym, "side": side, "client_id": cid or s.get("client_id", "UNKNOWN"), "session": sessionID}
    ...
    def _send_report(self, clord_id, cum, px, sym, side, status, last, orig_clord_id=None, text=None, sessionID=None):
        # Builds and sends ExecutionReport (35=8)
        try:
            tr = self.order_tracker.get(str(clord_id), {})
            target = sessionID or tr.get("session") or self.sessions.get(tr.get("client_id")) or self.active_session
            if not target: return
            er = fix44.ExecutionReport()
            er.setField(fix.ClOrdID(str(clord_id))), er.setField(fix.OrderID(str(clord_id))), er.setField(fix.ExecID(str(int(time.time() * 1000)))), er.setField(fix.Symbol(str(sym))), er.setField(fix.Side(str(side)))
            er.setField(fix.OrderQty(float(tr.get("total_qty", cum)))), er.setField(fix.CumQty(float(cum))), er.setField(fix.AvgPx(float(px))), er.setField(fix.LastQty(float(last))), er.setField(fix.LastPx(float(px))), er.setField(fix.LeavesQty(max(0.0, float(tr.get("total_qty", cum)) - float(cum))))
            if tr.get("client_id"): er.setField(fix.StringField(1, str(tr.get("client_id"))))
            er.setField(fix.ExecType({"0": "0", "1": "F", "2": "F", "4": "4", "5": "5", "8": "8"}.get(str(status), str(status))))
            er.setField(fix.OrdStatus({"0": "0", "1": "1", "2": "2", "4": "4", "5": "0", "8": "8"}.get(str(status), str(status))))
            if orig_clord_id: er.setField(fix.OrigClOrdID(str(orig_clord_id)))
            if text: er.setField(fix.Text(str(text)))
            fix.Session.sendToTarget(er, target)
            label = {"0": "ACK (NEW)", "1": "PARTIAL FILL", "2": "FULL FILL", "4": "CANCELLED", "5": "REPLACED", "8": "REJECTED"}.get(str(status), status)
            logger.info("📡 [REPORT] %s | ClOrdID=%s", label, clord_id)
        except Exception as e: logger.exception("_send_report failed: %s", e)
    ...
    def _send_cancel_reject(self, clord_id, orig_clord_id, reason, sessionID=None):
        ...
        try:
            ...
            rj.setField(fix.OrderID(str(orig_clord_id))), rj.setField(fix.ClOrdID(str(clord_id))), rj.setField(fix.OrigClOrdID(str(orig_clord_id))), rj.setField(fix.OrdStatus(fix.OrdStatus_REJECTED)), rj.setField(fix.CxlRejResponseTo(fix.CxlRejResponseTo_ORDER_CANCEL_REQUEST)), rj.setField(fix.Text(str(reason))), rj.setField(fix.TransactTime())
            fix.Session.sendToTarget(rj, target)
        except Exception as e: logger.exception("_send_cancel_reject failed: %s", e)
